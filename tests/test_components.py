...
def valid_streams():
    ...
    return {"inlets": {"air": inlet_air, "fuel": inlet_fuel}, "outlets": {"exhaust": outlet_exhaust}}
...
def test_combustion_chamber_calc_exergy_balance_success(combustion_chamber, valid_streams):
    ...
    total_E_P_in = (
        valid_streams["inlets"]["air"]["m"] * valid_streams["inlets"]["air"]["e_PH"]
    ...
    total_E_F_in = (
        valid_streams["inlets"]["air"]["m"] * valid_streams["inlets"]["air"]["e_CH"]
    ...
    assert pytest.approx(expected_E_P, rel=1e-3) == combustion_chamber.E_P
    assert pytest.approx(expected_E_F, rel=1e-3) == combustion_chamber.E_F
    assert pytest.approx(expected_E_D, rel=1e-3) == combustion_chamber.E_D
...
def valid_heat_exchanger_streams():
    ...
    return {"inlets": {0: inlet_hot, 1: inlet_cold}, "outlets": {0: outlet_hot, 1: outlet_cold}}
...
def test_heat_exchanger_calc_exergy_balance_success(heat_exchanger, valid_heat_exchanger_streams):
    ...
    assert pytest.approx(expected_E_P, rel=1e-3) == heat_exchanger.E_P
    assert pytest.approx(expected_E_F, rel=1e-3) == heat_exchanger.E_F
...
def valid_simple_hex_streams():
    ...
    return {"inlets": {0: inlet}, "outlets": {0: outlet}}
...
def test_simple_hex_calc_exergy_balance_success(simple_hex, valid_simple_hex_streams):
    ...
    assert pytest.approx(expected_E_P, rel=1e-3) == simple_hex.E_P
    assert pytest.approx(expected_E_F, rel=1e-3) == simple_hex.E_F
    assert pytest.approx(expected_E_D, rel=1e-3) == simple_hex.E_D
...
def deaerator_streams_outlet_above():
    ...
    """
    inlets = {0: {"T": 310, "m": 10, "e_PH": 900}, 1: {"T": 290, "m": 5, "e_PH": 800}}
    outlets = {0: {"T": 320, "m": 1, "e_PH": 950}}  # mass here is not used in calculation
...
def test_deaerator_outlet_above(deaerator, deaerator_streams_outlet_above):
    ...
    assert pytest.approx(expected_E_P, rel=1e-3) == deaerator.E_P
    assert pytest.approx(expected_E_F, rel=1e-3) == deaerator.E_F
    assert pytest.approx(expected_E_D, rel=1e-3) == deaerator.E_D
...
def deaerator_streams_outlet_equal():
    ...
    """
    inlets = {0: {"T": 310, "m": 10, "e_PH": 900}, 1: {"T": 290, "m": 5, "e_PH": 800}}
    outlets = {0: {"T": 300, "m": 1, "e_PH": 850}}  # e_PH here is irrelevant since T_out == T0
    return inlets, outlets
...
def test_deaerator_outlet_equal(deaerator, deaerator_streams_outlet_equal):
    ...
    assert pytest.approx(expected_E_F, rel=1e-3) == deaerator.E_F
    assert pytest.approx(expected_E_F, rel=1e-3) == deaerator.E_D
...
def test_drum_calc_exergy_balance_success():
    ...
    assert pytest.approx(expected_E_P, rel=1e-3) == drum.E_P
    assert pytest.approx(expected_E_F, rel=1e-3) == drum.E_F
    assert pytest.approx(expected_E_D, rel=1e-3) == drum.E_D
...
def test_mixer_calc_exergy_balance_success():
    ...
    mixer.inl = {0: {"T": 310, "m": 10, "e_PH": 800}, 1: {"T": 330, "m": 5, "e_PH": 850}}
    ...
    assert pytest.approx(expected_E_P, rel=1e-3) == mixer.E_P
    assert pytest.approx(expected_E_F, rel=1e-3) == mixer.E_F
    assert pytest.approx(expected_E_D, rel=1e-3) == mixer.E_D
...
def test_valve_all_above_ambient():
    ...
    assert pytest.approx(expected_E_F, rel=1e-3) == valve.E_F
    assert pytest.approx(expected_E_F, rel=1e-3) == valve.E_D
...
def test_valve_heat_release_split_true():
    ...
    assert pytest.approx(expected_E_P, rel=1e-3) == valve.E_P
    assert pytest.approx(expected_E_F, rel=1e-3) == valve.E_F
    assert pytest.approx(expected_E_D, rel=1e-3) == valve.E_D
...
def test_generator_calc_exergy_balance(generator):
    ...
    assert pytest.approx(expected_E_P, rel=1e-3) == generator.E_P
    assert pytest.approx(expected_E_F, rel=1e-3) == generator.E_F
    assert pytest.approx(expected_E_D, rel=1e-3) == generator.E_D
...
def test_motor_calc_exergy_balance_basic(motor):
    ...
    assert pytest.approx(100000, rel=1e-3) == motor.E_F
    assert pytest.approx(80000, rel=1e-3) == motor.E_P
    assert pytest.approx(20000, rel=1e-3) == motor.E_D
...
def test_compressor_case1_above_ambient(compressor):
    ...
    assert pytest.approx(expected_E_P, rel=1e-3) == compressor.E_P
    assert pytest.approx(expected_E_F, rel=1e-3) == compressor.E_F
    assert pytest.approx(expected_E_D, rel=1e-3) == compressor.E_D
...
def test_compressor_case3_both_below(compressor):
    ...
    assert pytest.approx(expected_P, rel=1e-3) == compressor.E_P
    assert pytest.approx(expected_E_F, rel=1e-3) == compressor.E_F
    assert pytest.approx(expected_E_D, rel=1e-3) == compressor.E_D
...
def test_pump_case1_above_ambient(pump):
    ...
    pump.inl = {0: {"T": 310, "m": m, "h": 500, "e_PH": 1000, "e_T": 900, "e_M": 150}}
    # Define outlet stream: temperature above ambient
    pump.outl = {0: {"T": 320, "m": m, "h": 600, "e_PH": 1100, "e_T": 950, "e_M": 160}}
    ...
    assert pytest.approx(E_P_expected, rel=1e-3) == pump.E_P
    assert pytest.approx(E_F_expected, rel=1e-3) == pump.E_F
    assert pytest.approx(E_D_expected, rel=1e-3) == pump.E_D
...
def test_pump_case2_inlet_below_outlet_above(pump):
    ...
    p0 = 101325
    m = 5
    pump.inl = {0: {"T": 290, "m": m, "h": 500, "e_PH": 1000, "e_T": 300, "e_M": 200}}
    pump.outl = {0: {"T": 310, "m": m, "h": 600, "e_PH": 1100, "e_T": 350, "e_M": 220}}
...
def test_pump_case3_below_ambient(pump):
    ...
    m = 5
    pump.inl = {0: {"T": 290, "m": m, "h": 500, "e_PH": 1000, "e_T": 300, "e_M": 200}}
    pump.outl = {0: {"T": 295, "m": m, "h": 600, "e_PH": 1100, "e_T": 350, "e_M": 220}}
    # Power: P = 5*(600-500) = 500 W.
    pump.calc_exergy_balance(T0, p0, split_physical_exergy=True)
...
def test_pump_invalid_case(pump):
    ...
    p0 = 101325
    m = 5
    pump.inl = {0: {"T": 320, "m": m, "h": 500, "e_PH": 1000, "e_T": 900, "e_M": 150}}
    pump.outl = {0: {"T": 310, "m": m, "h": 600, "e_PH": 1100, "e_T": 950, "e_M": 160}}
    pump.calc_exergy_balance(T0, p0, split_physical_exergy=True)
...
def test_storage_charging(storage, charging_streams):
    ...
    assert pytest.approx(600, rel=1e-6) == storage.E_F
    assert pytest.approx(400, rel=1e-6) == storage.E_P
    assert pytest.approx(200, rel=1e-6) == storage.E_D
...
def test_storage_discharging(storage, discharging_streams):
    ...
    assert pytest.approx(400, rel=1e-6) == storage.E_F
    assert pytest.approx(200, rel=1e-6) == storage.E_P
    assert pytest.approx(200, rel=1e-6) == storage.E_D
...
def valid_flash_streams():
    ...
    """
    inl = {0: {"m": 3, "e_PH": 100, "e_T": 50}, 1: {"m": 2, "e_PH": 80, "e_T": 40}}
    outl = {0: {"m": 1, "e_PH": 90, "e_T": 45}, 1: {"m": 4, "e_PH": 70, "e_T": 35}}
    return inl, outl
...
def test_flash_tank_calc_exergy_balance_success(flash_tank, valid_flash_streams):
    ...
    assert pytest.approx(expected_E_F, rel=1e-6) == flash_tank.E_F
    assert pytest.approx(expected_E_P, rel=1e-6) == flash_tank.E_P
    assert pytest.approx(expected_E_D, rel=1e-6) == flash_tank.E_D
...
def test_flash_tank_missing_streams_raises(flash_tank):
    ...
    # No inlets
    flash_tank.inl = {}
    flash_tank.outl = {0: {"m": 1, "e_PH": 90, "e_T": 45}, 1: {"m": 2, "e_PH": 80, "e_T": 40}}
    with pytest.raises(ValueError, match="Flash tank requires one inlet and two outlets."):
        flash_tank.calc_exergy_balance(T0=300, p0=101325, split_physical_exergy=True)

    # Only one outlet
    flash_tank.inl = {0: {"m": 1, "e_PH": 100, "e_T": 50}}
